...
try:
    from .usage_router_clean import track_usage_event
    from .proactive_router import track_user_activity
except ImportError:
    # Fallback if routers not available
    def track_usage_event(user_id: str, event_type: str, metadata: dict = None):
        pass
    def track_user_activity(user_id: str, activity_type: str, metadata: dict = None):
        pass
...
async def ask_question(
    ...
    try:
        ...
        sources = await rag.search_documents(request.question, top_k=5)  # Default top_k=5
...
async def get_conversation_history(
    ...
    """
    Get conversation history (compatible with frontend apiClient.js)
    """
    try:
        if conversation_id not in conversation_memory:
            return {
            ...
            messages.extend([
                {
                    ...
                    "text": turn.user_question,
                    "isUser": True,
                    "timestamp": turn.timestamp.isoformat()
                },
                {
                    "id": f"{conversation_id}_{len(messages)+1}",
